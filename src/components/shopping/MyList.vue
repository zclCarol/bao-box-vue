...
<template>
  <div id="myList" class="myList">
    <ul>
      <li v-for="item in list" :key="item.productId" @click="idToDetail(item)">
      ...
      <li v-show="isMore && list.length>=1" id="more">
        <!-- <router-link :to="{ path: '/realShoppingHome', query: {categoryID:categoryID} }">
          浏览更多
          <i class="el-icon-d-arrow-right"></i>
        </router-link> -->
          <div @click="callParentCallback()">
            浏览更多
            <i class="el-icon-d-arrow-right"></i>
          </div>      
      </li>
...
export default {
  ...
  methods: {
    //去详情页
    idToDetail(item){
      this.$emit('callbackToDetail',item)
    },
    //子组件回调父组件
    callParentCallback(){
      let curId = String(this.categoryID)
      this.$emit('callbackFunction',curId)
    },
...
.myList ul li img {
  ...
  width: 160px;
  height: 160px;
  background: url(../../assets/placeholder.png) no-repeat 50%;
  margin: 0 auto;
}
.myList ul li h2 {
  margin: 25px 10px 0;
