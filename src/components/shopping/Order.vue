      ...
      <div class="content" v-for="(item,index) in orders" :key="index">
        <ul>
          ...
          <li class="product-list" v-for="(product,i) in item" :key="i">
            ...
            <div class="pro-total pro-total-in">{{product.productPrice}}元</div>
          </li>
        </ul>
        <div class="order-bar">
...
export default {
  ...
  methods: {
    getOrders(){
      ...
      baoHttp.ajax.call(this, '/order/getOrders?userId='+userNo, {}, (data, err) => {
        if (!err && data.code == '0') {
    ...
    orders: function(val) {
      ...
      for (let i = 0; i < val.length; i++) {
        ...
        for (let j = 0; j < element.length; j++) {
          ...
          totalPrice += temp.productSellingPrice * temp.productNum;
        }
