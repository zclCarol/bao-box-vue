  ...
  <div class="goods" id="goods" name="goods">
    ...
    <div class="breadcrumb">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        <el-breadcrumb-item to="/realShoppingHome">首页</el-breadcrumb-item>
    ...
    <div class="main">
      <div class="list">
        <MyList :list="product" v-if="product.length>0" @callbackToDetail="callbackToDetail"></MyList>
...
export default {
  ...
  created() {
    // 获取分类列表
    this.getCategory()
    this.getData(this.goodsDefaultId)
  },
  activated() {
  ...
  methods: {
    ...
    // 向后端请求分类列表数据
    getCategory() {
      baoHttp.ajax.call(this, '/product/getAllCategory',{}, (data, err) => {
        if (!err) {
          const val = {
            categoryId: 0,
            categoryName: "全部"
          }
          const cate = data.AllCategory
          cate.unshift(val)
    ...
    getData(id) {
      ...
      let api = id == 0? "/product/getProducts": "/product/getProductsByCategoryId"
      api += '?page='+this.currentPage+'&pageSize='+this.pageSize
      let d = id == 0? {}:{categoryId:id}
