...
export default {
  ...
  methods: {
    ...
    deleteItem(e, id, productID) {
      baoHttp.ajax.call(this, '/product/delShoppingCart?cartId='+id+'&productId='+productID, {}, (data, err) => {
          if (!err) {
    ...
    //跳转到准订单页
    toOrder(){  
      let checkNum = this.$store.getters.getCheckNum
      if(checkNum > 0){
         this.$emit('handleCallbackToConfirmOrder')
      }
    }
  },
  computed: {
